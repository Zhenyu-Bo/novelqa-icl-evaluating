def build_transform_question_prompt(question: str) -> str:
    ...
    Your task is to transform the given question into a form that can be answered based on the content of a single chunk. And if the question is related to a specific chapter, you should also ask in which chapter(s) within this chunk the relevant content appears.
    Note: A single chunk may contain multiple chapters.
    ...
    4. If the question is related to a specific chapter(e.g., "Which chapter mentions Alice?"), ensure the transformed question asks whether the relevant content appears in this chunk, and if so, in which chapter(s) within this chunk it appears.
    ...
    Your output may be: "<answer>Does this chunk mention Alice? If so, in which chapter(s) within this chunk is Alice mentioned?</answer>"
...
def build_prompt_icl(chapter_content: str, question_options: str) -> str:
    ...
    5. If the question is related to a specific chapter (e.g., "Which chapter mentions Alice?"), indicate the chapter(s) within this chunk where the relevant content appears.
...
def build_prompt_final(question: str) -> str:
    ...
    1. Carefully review the answers and evidence for each chunk.
       ...
       - Exclude evidences that merely hint, suggest, or imply the answer. Only use evidence that clearly and unambiguously states the fact.
       - Note: A single chunk may contain multiple chapters. For each piece of evidence, if possible, indicate the chapter(s) within the chunk where the evidence appears.
    ...
    3. Before giving your final answer, list all supporting evidence:
       - Indicate the chunk(s) and, if possible, the chapter(s) within the chunk where the evidence is found.
    ...
    1. Chunk Evidence and Analysis:
       ...
       - Chunk X:
         - Evidence: [All direct and explicit evidence from this chunk. For each evidence, if possible, indicate the chapter(s) within the chunk.]
    ...
    1. Chunk Evidence and Analysis:
       - Chunk 1:
         - Evidence: "Quote1" (Chapter 2)
         - Analysis: "This directly shows..."
       - Chunk 2:
         - Evidence: "Quote2" (Chapter 5)
